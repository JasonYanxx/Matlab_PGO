...
function [params,pdf_overbound,cdf_overbound]=Gaussian_Pareto_bound(Xdata,x_lin)
    ...
    [params.thr_R,params.theta_R,params.xi_R,params.sigma_R]=gp_tail_overbound(Xdata);
    % left-tail overbound
    [params.thr_L,params.theta_L,params.xi_L,params.sigma_L]=gp_tail_overbound(-Xdata);
...
function [gmm_state]=geneStateGMM(gmm_dist,num)
    ...
    [P{1:num}] = ndgrid(p_group); % 使用ndgrid函数向量化循环
    ...
    prob_list = prob_c(:)'; % 将prob_c和sigma_c连接成一维数组
    sigma_list = sigma_c(:)';

    comps=length(sigma_list);
    sigma_all=num2cell(sigma_list);
    gmm_state = gmdistribution(zeros(comps,1), cat(3, sigma_all{:}), prob_list); % 生成state error 的GMM分布
end

function [FA_o,MD_o]=FDE_Gaussian(alpha,seed,num,gmm_dist,bias,sigma)
    % 生成观测数据（残差），每组6个观测，一共10000组 (false alarm)
    N=10000;
    T_o_arr=zeros(N,1);
    rng(seed);
    ...
    FA_o=sum(sum(T_o_arr>chi2inv(1-alpha,num-1)))/N; % 是否减去DOF
    
    % 每组6个观测（某个观测注入一定的bias），一共10000组 (miss detection)
    T_o_arr2=zeros(N,1);
    rng(seed);
    rk_all = random(gmm_dist, num*N);
    for i=1:N
    ...
    MD_o=1-sum(sum(T_o_arr2>chi2inv(1-alpha,num-1)))/N; % 是否减去DOF
end
...
function [FA_o,MD_o]=FDE_BayesGMM_seperate(alpha,seed,num,gmm_dist,bias,method)
    ...
    for i=1:N
        rk=rk_all((i-1)*num+1:i*num);       
        % 使用Bayes method (seperate approach) 计算统计量
        T1_list=[];
        for j=1:num
    ...
    if method == "sum"
        FA_o=sum(sum(T_o_arr_sum>chi2inv(1-alpha,num-1)))/N; % 是否减去DOF
    elseif method == "max"
    ...
    if method == "sum"
        MD_o=1-sum(sum(T_o_arr_sum2>chi2inv(1-alpha,num-1)))/N; % 是否减去DOF
    elseif method == "max"
